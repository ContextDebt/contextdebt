...
- Detection tiers:
  - **Tier 1** — two case-insensitive regexes. `MARKER` for confession words: workaround/hacky/HACK/hotfix/kludge/"until we upgrade"/etc. `MARKER_REMOVAL` for the intent to delete ("remove this when …", "will be removed in …"), which only counts inside a non-`WEAK` comment span because that wording is everywhere in UI strings. Both are guarded by a negation regex (`cannot/don't/never ... remove`) and by the comment-context rule below.
  - **Removal-intent shapes (v0.1.11)** — four additions, each inseparable from the guard that makes it safe. `we can use|replace X once|after …` requires the clause after `once`/`after` to start with we/this/it/they, or it swallows `delete the bucket once we flush` (an instruction to the program) and `will be replaced by Next.js` (a plan nobody dated). `TODO|FIXME|XXX … remove … once|after` requires the tag *and* the verb `remove` exactly, so `Delete source files after uploading` stays out. `Remove in v18.` requires the version to follow `in` directly, which leaves prose like `will be removed in Python 3.17` alone. `TODO(v11): remove,` reads the version out of the tag. Fixture rows `js_markers.js:19-22` are one per shape and `:23` is the negative case — do not "tidy" a guard away without watching those five.
  - **Quoted examples** — `quotedSpans()` rejects a marker (or a date) sitting inside a matching pair of `"` or `` ` `` on the same line: it is being cited, not confessed. `'` is not a delimiter (apostrophes), and a run of three is skipped as a docstring delimiter.
  - **Comment context** — a marker only counts inside a comment, in every supported language. `commentSpans(lang, line, state)` dispatches to `jsSpans` / `pySpans` / `phpSpans` / `liquidSpans`; `jsSpans` covers `.js/.jsx/.ts/.tsx/.mjs/.cjs/.mts/.cts` (the last two added in v0.1.11 — they were skipped entirely before, and nest's only marker lives in a `.mts` file); each returns `{ spans, state }` where spans are `[start, end)` index pairs and `state` carries across lines (open block comment, open docstring, inside `<?php`). A span may be flagged `WEAK` (docstrings): explicit marker words count there, removal intents do not.
  - **Tier 2** — GitHub issue status: `ISSUE_URL` regex on ±2-line context → `fetchIssue()` via api.github.com. Budget: 15 lookups unauthenticated, 100 with `GITHUB_TOKEN`. `reasonResolved()` decides whether the cited reason actually died; anything closed that it rejects goes to the separate `closedUnfixed` bucket.
...
- Excludes: `EXCLUDE_DIR` (node_modules, dist, build, tests, docs, …) and `EXCLUDE_FILE` (*.test.*, *.d.ts, *.min.js — the extension list there tracks `LANG`, `.mts`/`.cts` included). `build`/`dist`/`out` (`OUTPUT_DIR`) are still excluded at any depth — generated code would wreck precision — but the scan reports which ones it skipped, so a real source dir named `build` (CPython's `Tools/build`) is visible rather than silently missing; the escape hatch is scanning that path directly, since the root argument is never filtered. Reconsider before adding more names.
- Notes advisory (v0.1.10): `addressOf()` labels each marker with the kind of address it carries (`url`/`repo`/`self`/`date`) or null, and the closing `notes` block reports the two counts. Presentational only — it must never change what counts as a marker. Its copy has its own vocabulary rule: leads / address / come back; never verdict, expired, safe to delete, or dead code.
- Oracle status (v0.1.11): `--json` carries `issues_resolved`, `issues_unchecked`, `oracle_status` (`no_references`/`unavailable`/`partial`/`complete`) and `oracle_note`, and when nothing resolved, `expired_reasons` and `closed_unfixed` are **`null`, not `0`** — "we checked and found none" and "we could not check" are different claims, and printing both as zero was a quiet lie. The human report says it in words (*expired reasons: unknown, not zero*). Keep the null; a consumer counting on a number should be forced to notice.
...
- A bare `#N` is not a tracker reference. `ADDRESS_SELF` requires a keyword (`issue|issues|bug|ticket|pr|gh`) in front, because `#fff`, `#[Route(...)]` and "step #1" are not addresses. Widening it to a bare `#\d+` breaks four fixture rows on purpose.
- `quotedSpans()` must skip `"""` runs. Pairing them makes the whole docstring body a quoted span, which silently reverts the v0.1.7 docstring feature — the fixture suite catches it as `missing py_markers.py:9`, but only because that line is listed in `expected.json`.
...
- **Closed is not fixed** (v0.1.8). `state === "closed"` covers three endings: fixed, `not_planned`, and a PR closed unmerged. The last two mean the workaround is permanent — reporting them as EXPIRED was 73 of 752 closed references on the census corpus. The issues endpoint returns PRs too, with `pull_request.merged_at`, so `reasonResolved()` needs no extra request. Never widen it back to a bare state check.
