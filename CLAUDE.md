...
- Detection tiers:
  ...
  - **Comment context** — on `.py`/`.pyi` a marker must sit right of a `#`; on `.js`/`.jsx`/`.ts`/`.tsx`/`.mjs`/`.cjs` it must sit inside a comment span from `commentSpans()`, which tracks `/* */` across lines and skips string literals. PHP/Liquid have no rule yet.
...
- PHP and Liquid still have no comment-context rule (`$kludge = 0;` in .php flags). JS/TS and Python are covered as of v0.1.5/v0.1.6; PHP needs `#`, `//` and `/* */` together, plus the `<?php` boundary.
