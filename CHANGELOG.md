...
Precision first. On 1,340,156 lines the scanner produced exactly two `expired` verdicts and **both were wrong**, and on 270,525 lines of drizzle-orm it reported zero markers where a plain grep finds seventeen. This release is those two facts, fixed.

**The false claims.**

- **A version verdict inside somebody else's code is not a verdict about this project.** novu's `jsonSchemaFaker.js` carries `// TODO: remove in v2` inside a bundled copy of the `yaml` package; matched against `@novu/framework` 2.13.2 it read as expired, and the "v2" meant yaml@2, which shipped in 2021. Both of the only two expired verdicts across 1.34M lines were that one file. A file whose first 20 lines show provenance — copied/vendored/inlined/bundled from, a package CDN or release-artifact URL, a generated header, or a vendor-ish path — now makes every floor and release verdict in it **unresolved**, with the signal in `reason`. Read from comments only, so a file that merely mentions unpkg mid-code is untouched.
- **Prereleases compare properly.** `5.0.0-0` is *below* 5.0.0, not equal to it. The old comparison split on dots and read `"0-0"` as `NaN`, and the range reader dropped the prerelease before the comparison ever saw it. Both ends fixed, settled once for the own-version oracle and the dependency floor together. This is the nuxt false positive of 14 Sep.

**The misses.**

- **Removal intent in the active voice.** The family knew the passive (`will be removed in X`) and the conditional (`remove this when …`) but not the imperative, so `// TODO: remove` and `// TODO: remove?` were invisible — the whole reason drizzle-orm reported zero. Added a tag-plus-bare-`remove` shape and four objects that carry their own intent: `remove in future versions`, `remove as part of`, `remove and use X instead`, `remove sometime`. The tag is the guard, because `remove` is an ordinary English verb in UI copy, and `remove from` is excluded from it — that phrase belongs to the release-target shape, which requires a version-like target.
- **`remove this` now needs a reason to be a promise**: a `TODO`/`FIXME`/`HACK`/`XXX` tag, a condition (`when`/`once`/`after`/`if`/`unless`/`until`), or a stated intention (`we will …`, `later`, `eventually`, `in the future`). `// Remove this line not to show stack trace` and `/** Delete this message from the platform. */` describe what code does and are no longer markers.
- **A declaration whose name confesses is a marker too.** novu's `const esbuildDestructuringWorkaround` sits under a comment block naming the reason, the version that kills it and an upstream issue — with no marker word anywhere in it. The confession was in the identifier. A name ending in `workaround`/`hack`/`hotfix`/`shim`/`polyfill`/`patch`, or starting with the first three, counts when an adjacent comment block exists; that block becomes the `reason_span` and everything is read from it. **No comment, no marker** — `const hackyFix = {}` with no explanation is a style smell, and `removeTodo` is a function doing its job.
- **Floor conditions written in prose.** `Vite 7.3.3+ applies this automatically` and `vite >= 7.3.3` ask the same question `fixed in vite 5.1` asks. A comment naming several packages now has its claims pooled and the one this project actually depends on is chosen — novu's names esbuild and vite, and only vite is in the manifest.

**The noise.**

- **Generator-owned files are skipped whole.** A header saying `Code generated by … DO NOT EDIT` or `@generated` in the first five lines means the file is not scanned, not counted, and reported as `skipped_generated_files` with its directories. 999 of novu's files are one Speakeasy SDK, and they held 38 of its 101 findings. Nobody acts on those; the next refresh rewrites them.
- **Deprecation notices are counted apart.** A deprecation is intentional, scheduled and announced; a self-admitted workaround is an accident someone owned up to. `@deprecated` markers get their own bucket. And reasons are counted **unique** — novu's 101 findings held 48 distinct texts, two machine-written sentences accounting for 37 — so density is computed on unique reasons.

**Addresses, and a word for what we cannot check.** Linear issue URLs, Atlassian browse URLs and bare uppercase ticket keys are addresses now, with a denylist so `UTF-16` and `ISO-8601` are not mistaken for tickets. Only a GitHub URL keeps the ±2-line window; the rest are read from the marker's own line. Each finding carries **`verifiable`**: a Linear key is an address a human comes back for and nothing we run can ask about it — addressed and resolvable are two claims, made separately.

**An oracle that needs no API** (`--probe-upstream`, opt-in). `api.github.com` answers 403 from shared IPs while `git clone` answers 200, so for a reference the API cannot resolve, git alone can say whether any upstream commit claims that issue and which release contains it. Blob-less, no-checkout, five upstreams per scan, cached per run. It **never says open or closed** — "no commit mentions #233" is the absence of a claim, not evidence an issue is open — and the suite fails if a probe result ever carries the shape of a verdict.

**Measured**, v0.1.12 against this release, at the SHA each was taken:

| repo | SHA | markers | expired |
| --- | --- | --- | --- |
| novuhq/novu | `bc2776de3` | 101 → 63 | **2 → 0** (both were false) |
| drizzle-team/drizzle-orm | `b7862528f` | 0 → 18 | 0 |
| baptisteArno/typebot.io | `61056ff9a` | 2 → 3 | 0 |
| sindresorhus/np | `591e003bf` | 4 → 5 | 0 |
| mobxjs/mobx | `01211a698` | 2 → 2 | 0 |
| sindresorhus/got | `687eb7dcc` | 3 → 3 | 0 |

novu loses 40 lines: 38 in the generated SDK, and two that were never workarounds. mobx and got are the guard check and did not move at all. drizzle-orm is 18 where the hunt named 17 — the extra is `gel-core/columns/bigintT.ts:45`, `// TODO ts error if remove 'as string'`, which explains why the cast must stay rather than intending to delete it. It is a false positive, it is recorded as one, and **no guard was adjusted to make the number land on 17**.

**Fixtures** 53 → 79 lines, one directory per issue, each reproducing its bug in under 40 lines. `I_negatives` is the permanent guard — a `'Hack'` surname in a dataset, marketing copy, `removeTodo` — and must stay at zero. `readProbe` and the version comparison are pinned by offline tables, so the L2/L3 boundary and the prerelease rule are testable without a network.
